...
| Tarih | İşlem | Detay |
|-------|-------|-------|
| 2026-08-02 | **DD_GUARD öncesi [RISK-DEBUG] log eklendi** | `src/bot.py:728-733` — `_try_entry()`'de `is_defense_mode` sonrası, P1-13 DD_GUARD kontrolünden önce `get_current_dd()` + `log.info("[RISK-DEBUG] ... equity/peak/dd/broken")`. DD_GUARD tetiklenme anındaki portföy durumu log'da görünür. Yeni API yok. |
