"""analyzer_AVAXUSDT.py — HTF FVG Wick Rejection + Cift Yonlu Trailing SL/TP + Short Destek.
...
MIN_FVG_SIZE = 0.03  # AVAX specific
...
def run():
    ...
    if active_trades and current.is_closed:
        current_fvgs = detect_fvgs(
            chunk, lookback=min(50, len(chunk)), timeframe="15m", min_fvg_size=10.0
        )
    ...
    print(
        f" FVG buffer={FVG_BUFFER_MULT}x risk_pts | Session=NEWYORK | ADX yok | OB yok"
    )
    ...
    if trades:
        ...
        if trades:
            ...
            print(
                f" {'TP ile kapanan':<30}{tp_count} (%{tp_count/len(trades)*100:.1f})"
            )
            print(
                f" {'SL ile kapanan':<30}{sl_count} (%{sl_count/len(trades)*100:.1f})"
            )
