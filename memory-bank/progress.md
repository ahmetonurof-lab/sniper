...
| Tarih | İşlem | Detay |
|-------|-------|-------|
| 2026-08-02 | **DD_GUARD + [RISK-DEBUG] log kaldırıldı** | `src/bot.py` `_try_entry()` — P1-13 DD devre kesici entry engelleme guard'ı ve `44ee72d`'de eklenen `[RISK-DEBUG]` logu silindi. `is_defense_mode` da kaldırıldı (başka kullanımı yok). `RiskManager` state machine'i ve risk çarpanı aynen duruyor; `risk_manager.py` değişmedi. Suite: 190 passed (test_bot.py 15 fail pre-existing, kapsam dışı). |
