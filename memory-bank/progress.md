...
| Tarih | İşlem | Detay |
|-------|-------|-------|
| 2026-08-01 | **Cross-context bug fix turu — 13 bug, 12 commit** | `03e6eaf8` baz: BUG-1/7 (`_emergency_close` success + 4 call-site wrapper + mkt_side), BUG-25 (risk state `initial_equity` fallback + DD=%100 güvenli), BUG-23 (`should_trade` fail-closed), BUG-5 (ortak `cbdr_day_key`, K1=B), BUG-12 (idempotency `entry_order_id`, K2-A), BUG-8/2 (`ts_ms` msg['E'] öncelikli), BUG-21 (order_qty precision), BUG-10 (Decimal bump), BUG-11 (pending_exit tek blok), BUG-3 (trailing sl/tp canonical), BUG-17 (`is_open_async`), BUG-16 (dead code), BUG-29 (`setdefault`→`get`). Commitler: `5f08154..e244165`. |
