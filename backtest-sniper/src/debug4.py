...
for scan_bar in range(WINDOW, len(m15), 5):
    ...
    if ss.waiting_for_retrade and ss.trades_today == 1:
        ...
        if first_wait_ts is None:
            first_wait_ts = ts
        last_wait_ts = ts
        if wait_bars <= 5 or wait_bars % 100 == 0:
            print(
                f"  WAIT bar#{wait_bars} time={ts} want={ss.retrade_sweep_direction} cbdr_high={ss.cbdr_high:.2f} cbdr_low={ss.cbdr_low:.2f} cur_high={current.high:.2f} cur_low={current.low:.2f} close={current.close:.2f} cbdr_locked={ss.cbdr_locked} sweep_conf={ss.sweep_confirmed}"
            )

print(f"\nTotal wait bars: {wait_bars}")
print(f"First: {first_wait_ts}, Last: {last_wait_ts}")
