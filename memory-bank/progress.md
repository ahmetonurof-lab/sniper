...
| Tarih | İşlem | Detay |
|-------|-------|-------|
| 2026-08-06 | **Canlı teyit — log fix CANLI; SOL/ONDO korumasız (KRİTİK)** | (1) ✅ **KAPANDI:** yeni log (run `paper-20260805-212252`, restart 00:22:52, 552 satır) sıfır `[POST_ENTRY_DEBUG]`/`[P1-15_DEBUG]` → `bc73b5c` canlıda doğrulandı. (2) 🔴 **SOLUSDT-0 + ONDOUSDT-0 çıplak:** events: 00:03:47 ONDO orphan_cleaned (SL `1000000157572041`+TP `1000000157572047`), 00:04:24 SOL orphan_cleaned (SL `1000000157356807`+TP `1000000157356808`); restart sonrası hiçbir trade için koruma yerleştirme log'u yok; trailing her ~1dk `[TRAIL] trail#1 sl=74.092724 tp=75.092724` + eşzamanlı `trail_skipped no_better_trail_candidate` → koruma değişmiyor. Kök neden hipotezi: orphan_cleaned state'teki order_id'leri temizlemiyor → restart `recover_positions` yeniden koymuyor → trailing koruma eksikliğini yakalamıyor. Kullanıcıya acil manuel open-orders kontrolü iletildi. 🟡 P1-15 stale canlıda: 00:31:01 LINKUSDT SL stale event #1. Sıradaki: recovery/orphan kod doğrulaması + koruma restore fix. |
