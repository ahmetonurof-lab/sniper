...
def _detect_bar_direction(
    ...
    for _sp_abs, sp in swing_map.items():
        ...
        if (is_bull and bar_close > sp.price) or (not is_bull and bar_close < sp.price):
            if best_sp is None or (
                (is_bull and sp.price > best_sp.price)
                or (not is_bull and sp.price < best_sp.price)
            ):
                best_sp = sp
            mitigated.append(sp)
    ...
    penetration = (
        (bar_close - best_sp.price) if is_bull else (best_sp.price - bar_close)
    )
    ...
    if passes_size_filter:
        ...
        if prio != direction:
            ...
            local_bodies = [
                bars[x].body for x in range(body_start, bar_pos) if bars[x].is_closed
            ]
            local_avg_body = (
                sum(local_bodies) / len(local_bodies) if local_bodies else 0.0
            )
    ...
    if not passes_size_filter:
        logger.debug(
            "[CHoCH] %s veto @ %d — pivotlar korunuyor", direction.capitalize(), bar_abs
        )
        return None
    ...
    for fb in _bars_after[:sfp_n]:
        if not fb.is_closed:
            break
        if (is_bull and fb.close > best_sp.price) or (
            not is_bull and fb.close < best_sp.price
        ):
            _confirmations += 1
    ...
    logger.info(
        "[CHoCH] %s @ %d (level=%.5f)", direction.capitalize(), bar_abs, best_sp.price
    )
...
def detect_mss(
    ...
    active_high_map: dict[int, SwingPoint] = {
        p.bar_index: p for p in swing_mgr.active_highs()
    }
    active_low_map: dict[int, SwingPoint] = {
        p.bar_index: p for p in swing_mgr.active_lows()
    }
...
def create_mss_event(
    symbol: str, timeframe: str, direction: str, level: float, timestamp: int
) -> dict:
...
class LTFTriggerDetector:
    ...
    def validate(
        ...
        if len(bars) < self.atr_period + 2:
            result.reason = (
                f"[LTF-V1] Yetersiz bar: {len(bars)} < {self.atr_period + 2}"
            )
