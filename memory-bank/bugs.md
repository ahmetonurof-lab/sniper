...
### 🆕 A6 Denetim Raporu (2026-08-10) — Backtest-Live Parity Denetçisi

**Yöntem:** araştırma modu — KOD DEĞİŞİKLİĞİ YOK. Kanıt protokolü: her bulgu `KOD İZİ` / `TEST KANITI` / `VERİ KANITI` ile etiketlendi; kanıtlanamayanlar HİPOTEZ listesinde. MCP graf index'i bu repo için düşük kalite (yalnızca index.json düğümleri) → kullanıcı fallback kuralı gereği grep/read kullanıldı. `parity_check.py --check` = **PARITE_OK** — ancak bu araç yalnızca 2 fonksiyon çifti (`fvg_close_confirmed`/`_fvg_close_confirmed`, `fvg_confirm_mode`/`_fvg_confirm_mode`) + config sabitlerini AST-normalize karşılaştırıyor; aşağıdaki bulguların tamamı bu kapsamın DIŞINDA kalıyor (denetimin ek değeri bu).

**Motorlar:** canlı = `sniper/src/trading/trailing_manager.py` + `bot.py`; BT-1 = `backtest-sniper/src/analyzer_v5.py`; BT-2 = `sniper/simulate.py`. analyzer_v5 ve simulate.py canlı `config`/`session_router`/`fvg`/`retrace_state` modüllerini IMPORT ediyor (paylaşılan kod otomatik senkron; fark yalnızca yerel kopyalarda).

| ID | Şiddet | Canlı | Backtest | Senaryo / Koşul | Neden Bug | Kanıt | Önerilen Doğrulama |
|----|--------|-------|----------|------------------|-----------|-------|--------------------|
| **A6-01** | **HIGH** | `signal_engine.py:78-93` — `on_sweep(bar_index=current.index)` + çağrı sonrası `ss.sweep_confirmed=False` (satır 88; IDLE'da satır 93). 2026-08-06 SEIUSDT fix'i | `analyzer_v5.py:418-423` — `on_sweep(..., bar_index=None)`; dosyada `sweep_confirmed` hiçbir yerde `False`'a atanmıyor (yalnızca 418/426'da OKUNUYOR) | Aynı sweep her 15m bar'da yeniden onaylanabilir | `bar_index=None` → `retrace_state.py:106-109` `is_sweep_used` dedup'ı devre dışı; flag hiç tüketilmiyor → tek sweep çoklu giriş denemesi/pozisyon → backtest işlem sayısı canlıdan YÜKSEK şişer; canlıdaki SEIUSDT direction-fail döngüsü backtest'te HÂLÂ mevcut | KOD İZİ (iki taraf) + canlı fix kaydı `progress.md` 2026-08-06 + `test_integration.py:198-240` (sweep consumption testleri) | `analyzer_v5`'e aynı tüketim mantığını taşı; SEIUSDT fixture'ıyla tek-sweep-tek-giriş sayısını canlıyla karşılaştır |
| **A6-02** | **HIGH** | `bot.py:671` — `entry_price = current.close` (15m kapanışında giriş) | `analyzer_v5.py:477-478` + `simulate.py:235-237` — `ep = next_bar.open` (look-ahead karşıtı, kasıtlı) | Aynı sinyalde farklı entry fiyatı | SL/TP, risk_dist, qty, TP-shift, `be_triggered` hepsi entry çapasına bağlı → aynı sinyal farklı trade profili; canlıyla uzlaştırılmamış tasarım sapması | KOD İZİ | Karar noktası: backtest `sb.close`'a mı çekilmeli, canlı next-open'a mı? A/B replikasyon run'ı |
| **A6-03** | **MEDIUM** | `entry_manager.py:223-235` `apply_min_sl_distance` (`config.py:633` `MIN_SL_DISTANCE_TICKS=4`, `config.py:627` PCT) + `entry_manager.py:287-343` `validate_pre_entry_protection` (`config.py:639` `SL_EPSILON_TICKS=2`) + `bot.py:755-773` | `analyzer_v5.py:482-517`, `simulate.py:240-281` — SL buffer sonrası taban/clamp ve eps-reddi YOK | Dar SL'li sinyaller (SEIUSDT, ENA tipi) | Canlı reddeder/SL'i entry'den uzaklaştırır; backtest kabul eder → fazla trade + farklı SL/TP | KOD İZİ; `progress.md` 2026-08-06 zaten işaretli: "tick tabanı yalnızca canlıda — analyzer_v5'ye eklenmesi baş mühendisle görüşülecek" | Açık bilinen öğe; karar → backtest'e taşı veya kapsam dışı resmileştir |
| **A6-04** | **MEDIUM** | `trailing_manager.py:629-645` — `_fvg_multihop` tick_size verilince SL/TP tick grid'e normalize (`_norm`), çağrı `bot.py:349` | `analyzer_v5.py:639-701`, `simulate.py:354-381` — raw float SL/TP, normalizasyon YOK | Tick grid'i hop kararı/SL/TP kapanışını değiştirir | <1 tick kazançlar normalize'de yutulur → hop kararları farklılaşabilir; kayıtlı SL sub-tick farkı → exit fiyatı farkı | KOD İZİ; `trailing_manager.py:610-613` yorumu "evaluate_trail) raw davranış birebir korunur" — bilinçli ama belgelenmemiş tasarım | Backtest exchange_info tick_size ile aynı normalizasyonu uygula veya farkı resmi kabul et |
| **A6-05 | D-2 Fark 4** | LOW | `trailing_manager.py:698/714` — per-hop +1 | `analyzer_v5.py:740` `+ltc` per-hop ✓; `simulate.py:385` `+1` per bar ✗ | Bar başına çoklu hop | `simulate.py` FVG sayısına bakmıyor → trailing_count istatistiği şişer/azalır (be_triggered sınıfı >0 aynı kalır, raporlama sapar) | KOD İZİ | `simulate.py`'yi analyzer_v5 gibi `+ltc` yap |
| **A6-06 | D-2 Fark 3** | LOW | `signal_engine.py:112-115` — `is_closed` guard (açık bar'da trigger reddi) | `analyzer_v5.py:428` guard YOK; `simulate.py` live SignalEngine kullanıyor (simulate.py:45,225) → guard VAR | Kapalı-bar verisiyle tümü etkisiz; yalnızca replay/parsiyel veride | Açık bar'da trigger olabilir — gerçek backtest verisinde bar'lar kapalı olduğundan davranışsal fark YOK | KOD İZİ | Fark kapanana kadar düşük öncelik; replay mode eklenirse yeniden değerlendir |
| **A6-07** | LOW | `config.py:538` `ENTRY_VARIANT="A"`; canlı `bot.py`'de E-varyant dalı YOK (grep: ENTRY_VARIANT/choch bot.py'de 0 eşleşme) | `analyzer_v5.py:40,442-452` — E1/E2 CHoCH yön filtresi + overlap FVG seçimi (`_latest_choch`/`_pick_overlap_fvg` yalnızca burada) | Biri `ENTRY_VARIANT="E1"` yaparsa | Backtest filtreler, canlı sessizce yok sayar → gizli sapma. Şu an "A" → ölü kod, parity OK | KOD İZİ (canlıda fonksiyon yokluğu) | E1/E2 ya canlıya taşınmalı ya da config'ten kaldırılmalı |

**D-2 durum güncellemesi (bu denetimle kapananlar):**
- **Fark 1 (exit_now):** ✅ DÜZELTİLDİ (2026-07-26) — `parity_check.py --check` PARITE_OK ile teyit edildi.
- **Fark 2 (fvg_close_confirmed is_closed):** ✅ KAPANDI — artık İKİ tarafta da `is_closed` kontrolü YOK; fonksiyonlar AST-identik (parity_check PASS). Kapanış semantiği bar/close akışına devredilmiş.
- **Fark 5 (session filter):** ✅ KAPANDI — tek kaynak: analyzer_v5 ve simulate.py canlı `session_router`'ı import ediyor (`analyzer_v5.py:30-34`, `simulate.py:40-44`); formül birebir (`analyzer_v5.py:468` vs `signal_engine.py:140` — `(h>=sh or h<eh) if spans_midnight else ...`). `analyzer_v5.py:47` yerel `SESSION_HOURS` sabiti ÖLÜ (get_session_hours kullanılıyor).
- **D modu/continuation:** iki tarafta da `TRAIL_MODE=retrace` + ortak config; `analyzer_v5.py:702-736` atr-chase/activation fallback bloğu retrace'te ÖLÜ kod (güvenli). `TP_FIXED = getattr(cfg,"TRAIL_TP_FIXED",False)` → config'te yok → False → TP-shift her iki tarafta aynı (canlı `trailing_manager.py:697` vs `analyzer_v5.py:681-682`).

**HİPOTEZ listesi (kanıt yok, doğrulanmadı):**
- **H1 (A6-01):** sweep yeniden tetiklenmesinin gerçek veride ürettiği ekstra trade sayısı — tam run karşılaştırması gerekli.
- **H2 (A6-02):** close-vs-next-open farkının PnL/be_triggered sınıfı etkisi — fark genelde <1 bar hareketi, dar SL'lerde kritikleşir.
- **H3 (A6-04):** tick normalizasyonu canlıda daha AZ hop üretir (ALGOUSDT 08-08 kaydı destekliyor: 0.1 tick normalize tüm iyileşmeleri yutuyordu).
- **H4:** `execution_simulator.py` ana pipeline'da DEĞİL (yalnızca kendi testinde) — ileride bağlanırsa slippage paritesi ayrıca doğrulanmalı.
- **H5:** DENEYSEL config sabitleri (`CONT_BUFFER_MULT`, `TRAIL_ACTIVATION_R_MULT`, `ATR_TRAIL_MULT_CONTINUATION`, `CONTINUATION_CONFIRM_BARS`) iki tarafta da cfg.X ile okunuyor — değerler silinirse trailing_manager + analyzer_v5 senkron kaldırılmalı (progress.md 2026-08-08 notuyla aynı).

**Taranan dosyalar:** `sniper/src/trading/trailing_manager.py`, `sniper/src/trading/signal_engine.py`, `sniper/src/bot.py`, `sniper/src/config.py`, `sniper/src/entry_manager.py` (trading/), `sniper/src/risk_manager.py`, `sniper/src/session_router.py`, `sniper/src/session.py`, `sniper/src/fvg.py`, `sniper/src/retrace_state.py`, `sniper/src/state_manager.py`, `sniper/src/models.py`, `sniper/simulate.py`, `backtest-sniper/src/analyzer_v5.py`, `backtest-sniper/src/execution_simulator.py`, `backtest-sniper/parity_check.py` (çalıştırıldı: PARITE_OK).
