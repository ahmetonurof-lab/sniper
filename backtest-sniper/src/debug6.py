...
for scan_bar in range(WINDOW, len(m15), 5):
    ...
    if ss.waiting_for_retrade and ss.trades_today == 1:
        ...
        want = ss.retrade_sweep_direction
        tol = atr_val * 2.0
        if wait_count <= 10:
            if want == "bullish":
                print(
                    f"WAIT#{wait_count} {ts} want={want} cbdr_high={ss.cbdr_high:.2f} cur_high={current.high:.2f} diff={current.high - ss.cbdr_high:.2f} tol={tol:.2f} close={current.close:.2f} close<cbdr={current.close < ss.cbdr_high}"
                )
            else:
                print(
                    f"WAIT#{wait_count} {ts} want={want} cbdr_low={ss.cbdr_low:.2f} cur_low={current.low:.2f} diff={ss.cbdr_low - current.low:.2f} tol={tol:.2f} close={current.close:.2f} close>cbdr={current.close > ss.cbdr_low}"
                )

print(f"\nTotal wait bars: {wait_count}")
