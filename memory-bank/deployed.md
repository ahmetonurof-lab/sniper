...
|-------|--------|----------|
| 2026-08-17 13:06 | `4507e75` | **Binance Futures WS URL Migration (kritik fix).** Eski `/stream?streams=` sessizce veri düşürüyordu (6612 heartbeat timeout, 0 trade). Market: `/market/stream?streams=`, User data: testnet `/ws/<key>` / prod `private/ws?listenKey=<key>&events=...`. Screen 562472.bot, venv python. Teyit: 0 heartbeat timeout, USER_DATA bağlantısı kuruldu, INJUSDT short trade açıldı. |
